...
@Component
@Scope("prototype")
public class LoggedOutOpenByLeftLastEventConsumer extends SimpleEventsConsumer {
	...
	Logger logger = LoggerFactory.getLogger(LoggedOutOpenByLeftLastEventConsumer.class);
	
	public LoggedOutOpenByLeftLastEventConsumer() {
	}

	@Override
	public void consumerOperations(ConsumerRecord<String,String> record) {
    	try{
    		LoggedOutOpenByLeftLastEvent loggedOutOpenByLeftLastEvent = convertJsonBlobIntoEvent(record.value());    		
    		...
    		gameRoomClosedEvent.setUuid(loggedOutOpenByLeftLastEvent.getUuid());
    		gameRoomClosedEvent.setArrived(new Date());
    		gameRoomClosedEvent.setClazz("GameRoomClosedEvent");
    		gameRoomClosedEvent.setLoggedOutUserName(loggedOutOpenByLeftLastEvent.getOpenBy());
    		gameRoomClosedEvent.setGameRoom(loggedOutOpenByLeftLastEvent.getGameRoom());
	...
	private LoggedOutOpenByLeftLastEvent convertJsonBlobIntoEvent(String JsonBlob){
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			return objectMapper.readValue(JsonBlob, LoggedOutOpenByLeftLastEvent.class);
		} catch (IOException e) {
